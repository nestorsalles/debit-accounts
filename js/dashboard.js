        ...
        <div class="filter-bar">
          ...
          ${currentFilter === 'custom' ? `
            <div class="filter-date-range">
              ...
              <input type="date" id="filter-from" value="${customFrom || ''}"
                onchange="DH.dashboard.setCustomFrom(this.value)" max="${new Date().toISOString().split('T')[0]}">
              <span>${T('filter_to')}:</span>
              <input type="date" id="filter-to" value="${customTo || ''}"
                onchange="DH.dashboard.setCustomTo(this.value)" max="${new Date().toISOString().split('T')[0]}">
            </div>` : ''}
        </div>
